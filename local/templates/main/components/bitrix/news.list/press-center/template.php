    ...
    <div class="mc-common--div__GRID">
        ...
        <? foreach ($arResult["ITEMS"] as $arItem): ?>
            ...
            <article class="hm-pre--article__CARD <?= $class ?> DOWN"
                ...
                <a class="hm-pre--p__CARD_NAME" href="<?= $arItem['DETAIL_PAGE_URL'] ?>">
                    <?= $arItem["NAME"] ?>
                </a>
                <div class="hm-pre--div__CARD_BOTTOM">
                    <?
                    $tag = $arResult["PROPERTIES"]['TAG_KRASOTA']['DATA'][$arItem['PROPERTIES']['TAG_KRASOTA']['VALUE'][0]] ?? [];
                    ?>
                    <? if (!empty($tag)): ?>
                        <a class="hm-pre--a__CARD_TAG color-yellow" href="<?= $arItem['DETAIL_PAGE_URL'] ?>"
                           style="color:<?= $tag['UF_DESCRIPTION'] ?>">
                            <div class="hm-pre--div__CARD_TAG_CIRCLE" style="background-color: <?= $tag['UF_DESCRIPTION'] ?>"></div>
